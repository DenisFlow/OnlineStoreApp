...
class CartProductsActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var cartProductsUrl = "http://192.168.56.1//OnlineStoreApp/fetch_temprorary_order.php?email=${Person.email}"
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        if (item?.itemId == R.id.continueShoppingItem) {
            ...
            val delAllItems = "http://192.168.56.1//OnlineStoreApp/decline_order.php?email=${Person.email}"
            var requestQ = Volley.newRequestQueue(this@CartProductsActivity)
            var stringRequest = StringRequest(Request.Method.GET, delAllItems, Response.Listener{
                    response ->
                var intent = Intent(this, HomeScreen::class.java)
                startActivity(intent)
            ...
            var verifyOrderUrl = "http://192.168.56.1/OnlineStoreApp/verify_order.php?email=${Person.email}"
            var requestQ = Volley.newRequestQueue(this@CartProductsActivity)
